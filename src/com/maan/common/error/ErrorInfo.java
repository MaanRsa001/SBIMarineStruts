...
public class ErrorInfo extends OracleDateConversion
{
    ...
    */
    public void databasify(int mode) 
	{
        ...
        try {
            if (mode == MODE_UPDATE) {
                databasifyQuery = "UPDATE error_master SET error_desc =?, status = ? WHERE error_id = ?";

                if (con == null) setError("Connection Null");
                statement = con.createStatement();
                statement.executeUpdate(databasifyQuery,new String[] {getErrorDesc(),status,getErrorId()});
