        ...
        <s:form id="form1" name="form1" method="post" theme="simple">
			...
			<s:elseif test="'history'.equalsIgnoreCase(display)">
		 		<table align="center" width="100%">
		        	...
		        	<tr><td align="center">
			      	<display:table name="list" pagesize="10"  requestURI="<%=request.getContextPath()%>/linkoptionTravelConfigure.action" class="table" uid="row" id="record" style="align:center; " cellpadding="1" cellspacing="1"  varTotals="totals" >
					...
					<display:column sortable="true" style="text-align:center;" titleKey="SNo" value="<s:property value="record_rowNum"/>"/> 
...
{
if(from == 'edit'){
document.form1.action = "<%=request.getContextPath()%>/linkoptionTravelConfigure.action?optionId="+val+'&display='+from;
}else{
document.form1.action = "<%=request.getContextPath()%>/linkoptionTravelConfigure.action?display="+from;
}
document.form1.submit();
}
function fnScheme(val){
	postRequest('<%=request.getContextPath()%>/linkoptionTravelConfigure.action?reqFrom=scheme&productId='+val, 'schemeSelect');
}
function fnOption(val){
	postRequest('<%=request.getContextPath()%>/linkoptionTravelConfigure.action?reqFrom=option&schemeId='+val, 'optionSelect');
}
</script>
</div>
</div>
