	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<s:text name="Conveyance Type Selection" />
			</div>
				...
				<div class="row">
					<div style="width: 95%; margin: 0 auto;">
						<table width="100%" class="table table-bordered">
							<tbody>
							<s:iterator value="covertypeList" var="list" status="stat">
							<tr>
								<td>
									<%-- <input type="checkbox" name="coverType<s:property value='%{#list.ConveyanceCode}'/>" id="coverType<s:property value='%{#list.ConveyanceCode}'/>" value="<s:property value='%{#list.ConveyanceCode}'/>"/><s:property value="%{#list.ConveyanceDesc}"/> --%>
									<input type="checkbox" name="coverType<s:property value='%{#list.ConveyanceCode}'/>" id="coverType<s:property value='%{#list.ConveyanceCode}'/>" value="<s:property value='%{#list.ConveyanceCode}'/>~<s:property value='%{#list.ConveyanceDesc}'/>"/><s:property value="%{#list.ConveyanceDesc}"/> 
									<%-- <s:checkbox name="coverTypecheck[%{#stat.index}]" id="coverTypecheck_%{#list.ConveyanceCode}"></s:checkbox><s:property value="%{#list.ConveyanceDesc}"/>--%>
		            				<%-- <s:hidden name="coverTypeId[%{#stat.index}]" id="coverTypeId_%{#stat.index}" value="%{#list.ConveyanceCode}"></s:hidden>
									<s:hidden name="coverTypeName[%{#stat.index}]" id="coverTypeName_%{#stat.index}" value="%{#list.ConveyanceDesc}"></s:hidden> --%>
								</td>
							</tr>
							</s:iterator>
							</tbody>
...
<s:hidden name="coverTypeNames" id="coverTypeNames"/>
<s:hidden name="coverTypeIds" id="coverTypeIds"/>
...
function setValues()
{
	var coverTypeId=document.getElementById("coverTypeIds").value;
	var coverTypeName=document.getElementById("coverTypeNames").value;
	try
	{
...
getValues('<s:property value="coverTypeIds"/>')
