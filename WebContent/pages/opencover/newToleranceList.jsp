...
<script language="javascript">
	...
	function validate() {
		var docname = document.toleranceform;
		var statesList;
...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="panel panel-primary">
			...
			<s:text name="Tolerance Selection"></s:text>
			</div>
			<div class="panel-body">
				<div class="row">
			 		<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4"  align="center">
				 			<s:if test='includeList!=null && includeList.size()>0 '>
				 			<s:select name="leftSide" id="leftSide" list="includeList" listKey="Code" listValue="ToleranceName" headerValue="--- Available Tolerance ---" headerKey=""   multiple="true"  cssStyle="height:300px;width:200px"></s:select>
				 			</s:if>
				 			<s:elseif test="excludeList!=null && excludeList.size()>0 ">
				 			<s:select name="leftSide" id="leftSide" list="#{}"  headerValue="--- Available Tolerance ---" headerKey=""   multiple="true"  cssStyle="height:300px;width:250px"></s:select>
				 			</s:elseif>
				 			<s:else>
			 				<s:select name="leftSide" id="leftSide" list="toleranceList" listKey="Code" listValue="ToleranceName" headerValue="--- Available Tolerance ---" headerKey=""   multiple="true"  cssStyle="height:300px;width:200px"></s:select>
			 				
			 				</s:else>
			 		</div>
				 	<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4" align="center">
				 	<br/><br/>
			 				<input type="button" name="button3" value="  >  "  class="btn btn-sm btn-info" style="cursor: pointer;" onClick="javascript:add(document.toleranceform.leftSide,document.toleranceform.rightSide);"    >
								<br class="clear"/>
								<br class="clear"/>
								<input type="button" name="button4" value="  <  "  class="btn btn-sm btn-info" style="cursor: pointer;" onClick="javascript:add(document.toleranceform.rightSide,document.toleranceform.leftSide);"     >
								<br class="clear"/>
								<br class="clear"/>
								<input type="button" name="button1" value=" >> "   class="btn btn-sm btn-info" style="cursor: pointer;" onClick="javascript:moveAll(document.toleranceform.leftSide,document.toleranceform.rightSide);" >
								<br class="clear"/>
								<br class="clear"/>
								<input type="button" name="button2" value=" <<  "   class="btn btn-sm btn-info" style="cursor: pointer;" onClick="javascript:moveAll(document.toleranceform.rightSide,document.toleranceform.leftSide);" >
			 		</div>
			 		<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4"  align="center">
			 			<s:if test='excludeList!=null && excludeList.size()>0 '>
				 			<s:select name="rightSide" id="rightSide" list="excludeList" listKey="Code" listValue="ToleranceName" headerValue="--- Selected Tolerance ---" headerKey=""   multiple="true"  cssStyle="height:300px;width:200px"></s:select>
				 			</s:if>
				 			<s:else>
			 				<s:select name="rightSide" id="rightSide" list="#{ }" headerValue="--- Selected Tolerance ---" headerKey=""   multiple="true"  cssStyle="height:300px;width:200px"></s:select>
			 				</s:else>
			 		</div>
				</div>
				...
				<div class="row">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" align="center">
						...
						<input type="button" value="Submit" id="tolerancebtn" onclick="settingValues()" class="btn btn-sm btn-success">
					</div>
...
{
	if(document.Quotation.chkProposalNo.value == document.toleranceform.proposalNo.value)
	{
		if(document.toleranceform.rightSide.options.length<=1)
		{
		...
		else
		{
			...
			var rightlength	=	document.toleranceform.rightSide.length;

			for(i = 1; i < document.toleranceform.rightSide.options.length; i++) 
			{
				count++;
				var val	=0;
				val=document.toleranceform.rightSide.options[i].value;
			...
			document.toleranceform.tolerance.value=stNames;
			document.toleranceform.toleranceId.value=ids; 
			//document.toleranceform.action = 'saveToleranceOpenCover.action';
			//document.toleranceform.submit();
			//var URL='saveToleranceOpenCover.action?tolerance='+stNames+'&toleranceId='+ids;
			//alert(URL);
