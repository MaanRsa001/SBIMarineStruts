	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<s:text name="label.renewalPending" />
			</div>
			 		...
			 		<table class="table table-bordered table-stripped" id="gridTable" width="100%">
					 	<thead>
					 		<tr>
								...
								<th width="10%"><s:text name="label.renew"/></th>
								<th width="10%"><s:text name="label.delete"/></th>
							</tr>
					 	</thead>
						<tbody>
						   	<s:iterator value="quoteList" var="var" status="stat">
								<tr>
									...
									<td> <a href="#" class="two" title="Renew" onclick = "Renew('<s:property value="%{#var.ProposalNo}" />','<s:property value="%{#var.OpenCoverNo}" />')"> <b>Renew</b> </a> </td>
									<td> <a href="#" class="two" title="Delete" onclick = "ActiveDeActive('<s:property value="%{#var.ProposalNo}" />','D','')" ><b>Delete</b></a> </td>
								</tr>
							</s:iterator>
						</tbody>
					 </table>
...
<s:hidden name="deactive"/>
<s:hidden name="display"/>
</s:form>
<script type="text/javascript">
function quoteList(val){
	document.reportForm.action ='renewalPolicyReportReg.action';
	document.reportForm.submit();
}

function Renew(proposalNo, opencoverno)
{
	document.reportForm.proposalNo.value=proposalNo;
	document.reportForm.openCoverNo.value=opencoverno;
	document.reportForm.action ='renewOpenCover.action';
	document.reportForm.submit();
}
function ActiveDeActive(proposalNo,ad,adminStatus)
{
	...
	{
		if(ad == 'A')
			document.reportForm.deactive.value='active';
		else
		{
	...
	if(flag)
	{
		...
		document.reportForm.display.value='renew';
		document.reportForm.submit();
	}
