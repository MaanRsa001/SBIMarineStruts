		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<div class="body">
				    	<div class="row">
						    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
									<s:if test='warrId!=null && warrId.size()>0'>
										<table id="ConditionTable" class="table table-bordered-pop" border="2">
											<thead>
												<tr style="color:blue;">
													<th width="5%"><s:text name="label.Sno"/></th>
													<th width="5%"><s:text name="label.coverName"/></th>
													<th width="5%"><s:text name="label.warId"/></th>
													<th width="70%"><s:text name="label.warDescription"/></th>
													<%-- <th width="20%"><s:text name="label.commodity"/></th> --%>
												</tr>
											</thead>
											<tbody>
												<s:iterator value="warrId" var="list"  status="stat">
													<tr>
														<th align="center" width="10%"><s:property value="%{#stat.count}"/>
														<s:checkbox name="warcheck[%{#stat.index}]"   disabled="#disable"  ></s:checkbox>
														</th>
														<th align="left" width="5%">
															<s:property value='warcoverName[#stat.index]' />
															<s:hidden name="warcoverName[%{#stat.index}]"  id="warcoverName[%{#stat.index}]"></s:hidden>
															<s:hidden name="warcoverId[%{#stat.index}]"  id="warcoverId[%{#stat.index}]"></s:hidden>
														</th>
														<th align="left" width="5%">
															<s:property value='warrId[#stat.index]' />
															<s:hidden name="warrId[%{#stat.index}]"  id="warId[%{#stat.index}]"></s:hidden>
														</th>
														<th align="left" width="70%">
															<s:textarea name="warName[%{#stat.index}]"  id="warName[%{#stat.index}]" cssClass="form-control" cssStyle="width:100%" disabled="#disable"  onkeyup="textLimit(this,2000)"> </s:textarea>
														</th>
						    ...
						    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
									<s:if test='warrId!=null && warrId.size()>0'>
										<table id="ConditionTable" class="table table-bordered-pop" border="2">
											...
											<tbody>
												<s:iterator value="warrId" var="list"  status="stat">
												<s:if test='"true".equals(warcheck[#stat.index])'>
													<tr>
														<th align="center" width="10%"><s:property value="%{#stat.count}"/>
														</th>
														<th align="left" width="5%">
														...
														<th align="left" width="70%">
															<s:property value='warName[#stat.index]' />
														</th>
...
function fnSubmit(){
	document.Quotation.action = 'warPopupCondition.action';
...
function proceedBtn(){
	document.Quotation.action = 'addWarCondition.action';
	document.Quotation.submit();
}
function getCoverName(Field)
{
	document.Quotation.coverName.value=document.getElementById(""+Field).value;;
}
</script>	
</body>
</html>